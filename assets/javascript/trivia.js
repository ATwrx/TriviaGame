...
function initGame() {
    $("#welcome").hide();
    var gameTime = 6; //21 for actual game
    var currentQuestion = 0;
    var gameTimer = setInterval(runTrivia, 1000);
    var totalTime = 0; // Time Test
    function runTrivia() {
        // If it doesn't test variables first it won't reach currentQuestion = 10; 
        ...
        $("#question-num").html(currentQuestion + 1);
        questionSpace.text(triviaStr.question);
        jQuery.each(answerSet, function (i, val) {
            $("#" + i).text(i + ": " + val);
        });
        $("#game").show("slow"); // Works best here (after intial DOM update)
        $("#result-section").show();
